...
[ManulEngine](https://github.com/alexbeatnik/ManulEngine) (`pip install manul-engine`) is a separate Python runtime for deterministic, DSL-first web and desktop automation backed by Playwright. It is a different project from [ManulMcpServer](https://marketplace.visualstudio.com/items?itemName=manul-engine.manul-mcp-server) — ManulMcpServer is a VS Code extension that bridges Copilot chat to ManulEngine via a bundled Python runner. ManulAI connects to ManulEngine directly through its HTTP API.

When ManulEngine's HTTP server is running (`manul serve`, listening on `http://127.0.0.1:8000` by default), Agent Mode exposes these browser automation tools:

- `manul_run_step` — execute a single browser automation step (DSL or plain English)
- `manul_run_goal` — run a multi-step browser automation goal as a sequence of DSL steps
- `manul_scan_page` — scan the current page for interactive elements after navigation
- `manul_read_page_text` — read all visible text from the current browser page
- `manul_get_state` — get current ManulEngine session state (browser open, step count, last error)
- `manul_save_hunt` — save a `.hunt` automation script to disk
- `manul_run_hunt` — execute a full `.hunt` DSL document directly
- `manul_run_hunt_file` — read a `.hunt` file from disk and execute it

To use these tools, install ManulEngine and start the server:
...
ManulEngine uses the Hunt DSL — a deterministic, plain-English browser automation language. The agent mandate includes a full DSL reference covering all commands (NAVIGATE, Click, Fill, HOVER, Drag, VERIFY, EXTRACT, SCROLL, WAIT, and more), contextual qualifiers (NEAR, ON HEADER, ON FOOTER, INSIDE row with), and a mandatory VERIFY-after-every-action table. After completing any browser automation session, ManulAI always reconstructs the executed steps as a `.hunt` file preview and asks if the user wants to save it for later replay.

The ManulEngine base URL is configurable via `manulai.manulEngineBaseUrl` in `.manulai/settings.json`.
