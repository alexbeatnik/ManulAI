...
### Memory Law
All long-lived resources must be disposed correctly to avoid leaking memory, subprocesses, or Promises:
- Clear every `NodeJS.Timeout` on provider disposal.
- Abort every in-flight `AbortController` on disposal.
- Resolve any pending approval Promise so awaiting callers do not hang.
- Dispose the `ManulBridge` subprocess and all launched terminals on disposal.
- Guard all async entry points (`handleWebviewMessage`, `sendUserMessage`, `initializeSettingsState`, `postStateToWebview`) against use after `disposed`.
...
- Repeated-terminal-failure detection must cluster by a root-command signature, not just by exact command string. Models that hit a broken CLI (e.g. Tailwind v4 without `init`) tend to vary flags and subcommands across retries, so an exact-string key will never trip. Extract a root signature from the first token or runner+subject pair (`npx <pkg>`, `npm <subcommand>`, `pip install`, `cargo <subcommand>`, etc.) and nudge once the same root has failed across ≥2 distinct variations. The nudge must direct the model to read the installed package's docs / `bin` field, write the config manually, or switch to a different integration — not to vary arguments further. Root tracking must reset per user request and clear on any successful run of the same root.
