...
- For preferred-model greenfield tasks, reject shallow placeholder scaffolds, reject overly thin first Python files, and do not allow `execute_terminal_command` immediately after the first successful `create_or_edit_file`; require another real file/tool step or a direct completion instead.
