...
- `create_or_edit_file` must auto-create missing parent directories (`vscode.workspace.fs.createDirectory` on the parent) before writing. Greenfield prompts routinely write nested paths like `src/index.ts` in a fresh project; failing with `ENOENT` instead of creating the parent forces a redundant "first call mkdir" turn and confuses weak models.
- `replace_in_file` must error on multi-match instead of silently replacing the first occurrence. A short `old_text` that appears more than once (e.g. `return 1` in both branches of an `if`/`else`) is almost always ambiguous: replacing the first lets the model "fix" something while corrupting unrelated code. Force the model to add surrounding context until the match is unique.
- `replace_in_file` must reject calls where `old_text === new_text`. The replace would make no change, so accepting it lets the model claim work was done when nothing happened.
...
- Detect refusal-style responses: when the user prompt contains action verbs (create/edit/rename/fix/etc.) and the model's response has zero tool calls AND no malformed tool-call payload AND no successful tool execution has occurred yet, treat that as a refusal and nudge the model once to actually call a tool before accepting the response as a final answer. Suppress the nudge for purely conversational prompts (greetings, "explain this code") and for the second-or-later turn so the model can produce a real summary after work is done.
- Detect description / explanation intent: when the user prompt matches `tell me about | describe | explain | what is | summarize | how does | про що | опиши | розкажи` AND does NOT contain action verbs, mark it as explain-only. For explain-only prompts: (1) inject a system-prompt hint telling the model to read documentation files (`README.md`, `AGENTS.md`, `CLAUDE.md`, `package.json`) before answering and to avoid reading random source files; (2) block `execute_terminal_command` and `launch_in_terminal` at the dispatch layer with a clear error so the model is forced back to doc-reading. Observed bug otherwise: a model on "tell me about this project" runs a random script, then reads its source, then describes the script instead of the project.
- Cap tools per turn at 3 (`MAX_TOOLS_PER_TURN`). Some models emit 20+ tool calls in a single response (observed with `qwen3-coder:30b` on broad "scan and explain" prompts), which blows up context and produces redundant work. When the model exceeds the cap, prioritise writes (`create_or_edit_file`, `replace_in_file`, `delete_file`) first, then terminal commands (`execute_terminal_command`, `launch_in_terminal`), then reads. Also de-duplicate identical `(tool, args)` pairs WITHIN the same turn before counting against the cap.
