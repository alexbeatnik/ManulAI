...
- For preferred-model greenfield tasks, reject shallow placeholder scaffolds, reject overly thin first source files, recover valid plain-text code dumps into real file writes when that is safer than another retry loop, and do not allow `execute_terminal_command` before the first real source-file write or immediately after the first successful `create_or_edit_file`; require another real file/tool step or a direct completion instead.
